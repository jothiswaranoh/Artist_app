  ...
  return (
    <div className="artist-page">
      ...
      {currentUser?.role === "artist" && (
        ...
        >
          <User size={16} style={{ color: "#ED80E9", flexShrink: 0 }} />
          <span
            style={{
              fontSize: "0.8rem",
          ...
          >
            Your display name (shown to customers):
          </span>
          <input
            value={displayName}
            onChange={(e) => {
      ...
      <div className="stats-row">
          ...
          <div>
            ...
            <p className="stat-value">
              {serviceOfferings.length > 0
                ? `₹${Math.min(...serviceOfferings.map((s: ServiceOffering) => s.price || 0))} - ₹${Math.max(...serviceOfferings.map((s: ServiceOffering) => s.price || 0))}`
                : "₹0"}
            </p>
          </div>
        </motion.div>
        <motion.div className="stat-card" whileHover={{ y: -2 }}>
          ...
          <div>
            ...
            <p className="stat-value">
              {serviceOfferings.length > 0
                ? `${Math.round(serviceOfferings.reduce((s: number, x: ServiceOffering) => s + (x.duration_minutes || 0), 0) / serviceOfferings.length)} min`
                : "0 min"}
            </p>
          </div>
        </motion.div>
